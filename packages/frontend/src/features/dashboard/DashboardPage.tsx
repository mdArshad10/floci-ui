...
import {AlertTriangle, CheckCircle2, ExternalLink, Info, RefreshCw, Zap,} from 'lucide-react'
...
function StatsBar({
    ...
    const activeServices = (data?.health.services ?? []).filter((s) => s.status === 'healthy').length
    const totalServices = SERVICE_META.filter((m) => m.implemented).length
    ...
    return (
      ...
      <span className="stats-bar-item">
        ...
        <span>of {totalServices} services active</span>
      </span>
            <span className="stats-bar-item">
        ...
        <span>total resources</span>
          ...
          <span>log groups</span>
            ...
            <button className="stats-bar-refresh" onClick={onRefresh} disabled={isLoading}>
...
function ServiceCard({
    ...
    const status = svcInfo?.status ?? (meta.implemented ? 'unknown' : 'unavailable')
                ...
                <div style={{marginTop: 14}}>
                    ...
                    {latency !== undefined && (
                        <div style={{marginTop: 10, fontSize: 11, color: '#5f7080'}}>
                            API latency <span style={{color: '#d1d1d1'}}>{formatLatency(latency)}</span>
                        </div>
                    )}
                </div>
            ) : (
...
function QuickActions() {
    ...
    return (
        <div className="grid">
            {actions.map((action) => (
                <button key={action.route} className="link-button" onClick={() => navigate(action.route)}>
...
function Widget({title, footer, children}: { title: string; footer?: React.ReactNode; children: React.ReactNode }) {
    return (
        <section className="widget">
            <div className="widget-header">
                <h3>{title}</h3>
                {footer === undefined ? null : <span style={{marginLeft: 'auto'}}>{footer}</span>}
            </div>
...
export function DashboardPage() {
    ...
    return (
        <>
            ...
            <StatsBar data={data} onRefresh={() => void refetch()} isLoading={isLoading}/>

            <div className="content">
                {isError && (
                    <div className="card" style={{padding: 14, marginBottom: 16, borderColor: 'rgba(239,68,68,.28)'}}>
            <span className="status unavailable">
              Cannot reach Floci. Make sure it is running at the configured endpoint.
            </span>
                    </div>
                )}
                ...
                <div className="grid two">
                    <Widget
                        title="Floci Health"
                        footer={<span
                            className="link-button">{serviceErrors > 0 ? `${serviceErrors} error${serviceErrors > 1 ? 's' : ''}` : 'All systems operational'}</span>}
                    >
                        <div className="metric-grid">
                            <div>
                                <p className="metric-label">Endpoint issues</p>
                                <p className={`metric-value ${isError || serviceErrors ? 'status unavailable' : ''}`}>
                            ...
                            <div>
                                ...
                                <p className="metric-value">
                                    {isLoading
                                        ? '—'
                                        : `${(data?.health.services ?? []).filter((s) => s.status === 'healthy').length} of ${implementedMeta.length}`}
                                </p>
                            </div>
                            <div>
                                <p className="metric-label">Total resources</p>
                                <p className="metric-value">{formatNumber(data?.totalResourceCount)}</p>
                            </div>
                            <div>
                                <p className="metric-label">Last checked</p>
                                <p className="metric-value" style={{fontSize: 12}}>{timeAgo(data?.checkedAt)}</p>
                            </div>
                        </div>
                    </Widget>
                    ...
                    <Widget
                        title="CloudWatch"
                        footer={<Link className="link-button" to="/cloudwatch">Open CloudWatch <ExternalLink size={11}/></Link>}
                    >
                        <div className="metric-grid six">
                            <div>
                                <p className="metric-label">Log groups</p>
                                <p className="metric-value">{formatNumber(data?.logGroupCount)}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p className="metric-value">{formatNumber(data?.metricCount)}</p>
                            </div>
                            <div>
